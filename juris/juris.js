                         ...
                         if (instance.hooks.onMount) {
                              setTimeout(() => {
                                   try {
                                        ...
                                        if (mountResult?.then) {
                                             promisify(mountResult).catch((error) => log.ee && console.error(log.e(`Async onMount error in ${name}:`, error), "application"));
                                        }
                                   } catch (error) {
                              ...
                              if (instance.hooks.onMount) {
                                   setTimeout(() => {
                                        try {
                                             const mountResult = instance.hooks.onMount();
                                             if (mountResult?.then) {
                                                  promisify(mountResult).catch(
                                                       (error) => log.ee && console.error(log.e(`Async onMount error in ${instance.name}:`, error), "application")
                                                  );
                                             }
                                        } catch (error) {
...
class Juris {
               ...
               (requestIdleCallback || setTimeout)(() => {
                    ...
                    for (let key in globalThis) {
                         if (globalThis[key] instanceof Juris) {
                              log.ew && console.warn(`âš ï¸ JURIS GLOBAL: '${key}'`);
                         }
                    }
               });
          }
     ...
     getHeadlessAPI(name) {
          return this.headlessManager?.getAPI(name);
     }
               ...
               if (Array.isArray(this.layout)) {
                    ...
                    if (hasReactiveFunctions) {
                         ...
                         log.ei &&
                              console.info(
                                   log.i(
                                        "Render completed (reactive array)",
                                        {
     ...
     arm(target, handlerFn) {
          ...
          const armedInstance = {
               ...
               trigger(eventName, eventData = {}) {
                    const listener = eventListeners.find((e) => e.original === eventName || e.actual === eventName);
